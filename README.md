...
또한 orphan이 아니더라도 `Codex app-server` 아래의 오래된 MCP/tool cohort, `Claude.app` 아래의 오래된 helper cohort, 그리고 오래 남은 공통 언어 서버/MCP launcher helper가 중복으로 누적되는 경우를 위해, opt-in 모드로 stale helper dedupe를 켤 수 있습니다.
...
Codex helper cohort dedupe까지 자동화하려면:

```bash
./install.sh --stale-codex-cohorts
```

Claude helper dedupe까지 자동화하려면:

```bash
./install.sh --stale-claude-helpers
```

전체적인 공통 helper dedupe까지 자동화하려면:

```bash
./install.sh --stale-generic-helpers
```

둘 다 함께 켜려면:

```bash
./install.sh --aggressive --stale-codex-cohorts --stale-claude-helpers --stale-generic-helpers
```
...
Codex stale cohort 모드 드라이런:

```bash
~/bin/clean-ai-orphans.sh --dry-run --verbose --stale-codex-cohorts
```

Claude stale helper 모드 드라이런:

```bash
~/bin/clean-ai-orphans.sh --dry-run --verbose --stale-claude-helpers
```

공통 helper 모드 드라이런:

```bash
~/bin/clean-ai-orphans.sh --dry-run --verbose --stale-generic-helpers
```
...
현재 `--stale-codex-cohorts` 모드가 추가로 정리하는 대상은 아래와 같습니다.

- `Codex.app` 본체는 건드리지 않고, `codex app-server` 직계 자식 중 오래된 MCP/tool launch cohort만 대상
- helper family별로 최신 2개 cohort는 남기고 더 오래된 중복 cohort만 정리
- 대상 family 예시: `trace-server`, `wiki-server`, `state-server`, `memory-server`, `code-intel-server`, `serena`, `context7`, `playwright`

현재 `--stale-claude-helpers` 모드가 추가로 정리하는 대상은 아래와 같습니다.

- `Claude.app` 본체와 renderer/gpu/helper utility는 건드리지 않고, 오래된 direct wrapper/helper family만 대상
- helper family별로 최신 2개 cohort는 남기고, CPU가 낮은 오래된 중복 helper만 정리
- 대상 family 예시: Claude extension wrapper, `kmsg-mcp.py`

현재 `--stale-generic-helpers` 모드가 추가로 정리하는 대상은 아래와 같습니다.

- 특정 앱 전용 규칙과 겹치지 않는 공통 helper family만 대상
- helper family별로 최신 2개 cohort는 남기고, CPU가 낮은 오래된 중복 helper만 정리
- 대상 family 예시: `typescript-language-server`, `tsserver.js`, `typingsInstaller.js`, `context7`, `playwright`, `serena`
